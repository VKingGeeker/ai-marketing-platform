...
@Injectable()
export class MoonshotService implements AIBaseService {
  ...
  constructor(
    ...
    this.model = this.configService.moonshotModel;
    this.timeout = this.configService.aiRequestTimeout;
    this.maxContentLength = this.configService.aiMaxContentLength;

    this.client = axios.create({
      baseURL: this.configService.moonshotBaseUrl,
      timeout: this.timeout,
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${this.configService.moonshotApiKey}`,
      },
    });
  }
  ...
  async generateMarketingCopy(params: MarketingCopyParams): Promise<AIResponse> {
    ...
    try {
      ...
      this.logger.error(`Moonshot 生成营销文案失败: ${error}`);
  ...
  async generateProductDesc(params: ProductDescParams): Promise<AIResponse> {
    ...
    try {
      ...
      this.logger.error(`Moonshot 生成产品描述失败: ${error}`);
      throw new AIServiceException(
        '产品描述生成失败',
        this.getProviderName(),
        error?.response?.status,
  ...
  async generateSocialContent(params: SocialContentParams): Promise<AIResponse> {
    ...
    try {
      ...
      this.logger.error(`Moonshot 生成社交媒体内容失败: ${error}`);
      throw new AIServiceException(
        '社交媒体内容生成失败',
        this.getProviderName(),
        error?.response?.status,
  ...
  private async callAI(prompt: string): Promise<string> {
    const apiKey = this.configService.moonshotApiKey;

    if (!apiKey || apiKey === 'your-moonshot-api-key') {
      throw new AIServiceException(
        'Moonshot API Key 未配置',
        this.getProviderName(),
      );
    }
